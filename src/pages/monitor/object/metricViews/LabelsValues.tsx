...
export default function LabelsValues(props: IProps) {
  ...
  const [dimensionLabelsValues, setDimensionLabelsValues] = useState<{
    [key: string]: string[];
  }>({});
  ...
  return <div className='n9e-metric-views-labels-values'>
      {!_.isEmpty(filtersStr) && <div className='n9e-metric-views-labels-values-item'>
        ...
        setExpaned({ ...expaned,
          filters: !expaned.filters
            ...
            {t("前置过滤条件")}{expaned.filters ? <UpOutlined /> : <DownOutlined />}
          </div>
          {expaned.filters && <div className='n9e-metric-views-filters'>{filtersStr ? filtersStr : t("暂无数据")}</div>}
        </div>}
      {!_.isEmpty(dynamicLabels) && <div className='n9e-metric-views-labels-values-item'>
          <div className='page-title' style={{
        cursor: 'pointer'
      }} onClick={() => {
        setExpaned({ ...expaned,
          dynamicLabels: !expaned.dynamicLabels
            ...
            {t("动态过滤条件")}{expaned.dynamicLabels ? <UpOutlined /> : <DownOutlined />}
          </div>
          {expaned.dynamicLabels && <div className='n9e-metric-views-dynamicLabels'>
              {_.isEmpty(dynamicLabels) ? <div style={{
          marginBottom: 10
        }}>{t("暂无数据")}</div> : _.map(dynamicLabels, item => {
          const {
            t
          } = useTranslation();
          return <div key={item.label} className='n9e-metric-views-dynamicLabels-item'>
                      <div className='n9e-metric-views-dynamicLabels-item-label'>{item.label}:</div>
                      <Select allowClear showSearch style={{
              width: '100%'
            }} value={item.value} onChange={val => {
      ...
      return <div key={dimensionLabel.label} className='n9e-metric-views-labels-values-item'>
              ...
              onChange({ ...value,
                dimensionLabels: _.map(dimensionLabels, item => {
                  if (item.label === dimensionLabel.label) {
                    return { ...item,
                      value: dimensionLabelsSearch[item.label] ? _.filter(dimensionLabelValues, o => {
            ...
            <div className='n9e-metric-views-dimensionLabel'>
              <Input.Group compact>
                <Input style={{
              width: 'calc(100% - 32px)'
            }} prefix={<SearchOutlined />} value={dimensionLabelsSearch[dimensionLabel.label]} onChange={e => {
              setDimensionLabelsSearch({ ...dimensionLabelsSearch,
                [dimensionLabel.label]: e.target.value
              });
            }} />
                <Tooltip title={t("清空已选的值")} placement='right' getTooltipContainer={() => document.body}>
              ...
              <div className='n9e-metric-views-dimensionLabel-content'>
                {_.isEmpty(dimensionLabelValues) ? t("暂无数据") : <div>
                    {_.map(_.filter(dimensionLabelValues, item => {
                let result = true;
