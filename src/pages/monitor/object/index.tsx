...
export default () => {
  ...
  const getContent = (graph, i) => {
    return <div>
      <Checkbox onChange={() => {}}>Multi Series in Tooltip, order value desc</Checkbox><br/>
      <Checkbox onChange={(e) => {
        let newGraphs = [...graphs]
        let dstGraph = newGraphs[i]
        dstGraph.legend = e.target.checked
        setGraphs(newGraphs)
      }}>Show Legend</Checkbox><br/>
  ...
  return <PageLayout title={t('对象视角')} icon={<LineChartOutlined />}>
    <div className='object-view'>
      <Layout style={{ padding: 10 }}>
        <Row gutter={10}>
          <Col span={12}>
            <HostSelect allHosts={hosts} changeSelectedHosts={(hosts) => {
        ...
        <div>
          {_.map(graphs, (o, i) => {
            return <div style={{ marginBottom: 10 }}>
              <Graph ref={o.ref} data={o} key={i} timeVal={timeVal} extraRender={graph => {
                return [
