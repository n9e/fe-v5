...
export default (props => {
  ...
  const renderMetricList = (metrics = [], metricTabKey: string) => {
    return <div className='tabPane' style={{
      ...
      overflow: 'auto'
        ...
        {metrics.length ? <ul className='metric-list' style={{
        border: 'none'
      }}>
          ...
          return <li className='item' key={i} onClick={() => {
            handleMetricClick && handleMetricClick(metric);
                  ...
                  {selectedMetrics.find(sm => sm === metric) ? <span style={{
              marginLeft: 8
            }}>+1</span> : null}
                </li>;
        })}
  ...
  const renderMetricTabs = () => {
    let filteredMetrics = normalizMetrics(activeKey); // _.forEachRight(selectedMetrics, (value) => {
    ...
    const tabPanes = _.map(metricPrefixesUnique, val => {
      return <TabPane tab={val} key={val}>
          {renderMetricList(newMetrics, val)}
        </TabPane>;
    });

    tabPanes.unshift(<TabPane tab='all' key='all'>
        {renderMetricList(newMetrics, 'all')}
      </TabPane>);
    return metrics.length > 0 ? <Tabs className='metric-tab' tabBarStyle={{
      marginBottom: 10,
      height: 40
    }} activeKey={activeKey} onChange={handleMetricTabsChange}>
        {tabPanes}
      </Tabs> : <div className='metric-tab-empty'>
        {t("请先选择监控对象，然后")}后{' '}
  ...
  return <div className='metric-select'>
      <div className='top-bar'>
        <Input placeholder={t("搜索，空格分隔多个关键字")} addonBefore={<>
              <div className='search-title'>{t("监控指标")}</div>
