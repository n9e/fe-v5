...
export default (props => {
  ...
  const [isModalVisible, setIsModalVisible] = useState(false); // 只用作弹框出来以后selectedHosts属性变化触发的setShowHosts
  ...
  const columns = [{
    ...
    title: t("操作"),
    width: 80,
    render: (t, h) => <Button type="link" block style={{
      padding: 0,
      textAlign: 'left'
    }} onClick={t => {
  ...
  return <>
    <Button type='link' style={{
      ...
      showModal();
    }}>{t("已选中（")}{selectedHostsKeys.length}/{allHosts.length}）</Button>
    <Modal width={800} title={`${t("已选中监控对象（")}/${selectedHostsKeys.length}）${allHosts.length}`} visible={isModalVisible} onOk={handleOk} onCancel={handleCancel} footer={null}>
      <div style={{
        display: 'flex',
        ...
        <Input style={{
          width: 200
        }} placeholder={t("搜索标识、标签、备注")} onPressEnter={e => {
        ...
        <Button danger onClick={_ => changeSelectedHosts([])}>{t("清空所有选中的对象")}</Button>
      </div>
      <Table style={{
        marginTop: 10
      }} className='host-list' rowKey='ident' columns={columns} dataSource={showHosts} pagination={{
        simple: true
      }}></Table>
    </Modal>
  </>;
