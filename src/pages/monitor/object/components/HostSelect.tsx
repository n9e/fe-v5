...
export default (props => {
  ...
  const columns = [{
    ...
    render(tagArr) {
      return tagArr && tagArr.map(item => <Tag color='purple' key={item}>
              {item}
            </Tag>);
    }
  ...
  const selectBefore = <div className='host-add-on'>
      <div className='title'>{t("监控对象")}</div>
      <div className='select-before'>
        <Select allowClear placeholder={t("按业务组筛选")} value={curSelectBusiGroup} dropdownMatchSelectWidth={false} style={{
        width: '100%',
        textAlign: 'left'
      }} showSearch={true} filterOption={false} onSearch={handleSearch} onFocus={() => {
        ...
        setCurBusiItemInHostSelect(data);
          ...
          {filteredBusiGroups.map(bg => <Option key={bg.id} value={String(bg.id)}>
              {bg.name}
            </Option>)}
        </Select>
      </div>
  ...
  return <div className='host-select'>
      <div className='top-bar'>
        <Input placeholder={t("搜索，空格分隔多个关键字")} addonBefore={selectBefore} className='host-input' onPressEnter={e => {
        const {
          value
      ...
      <Table className={allHosts.length > 0 ? 'host-list' : 'host-list-empty'} rowKey='ident' rowSelection={{
    ...
    }} columns={columns} dataSource={allHosts} pagination={{
      simple: true
    }}></Table>
      <div style={{
      marginTop: -44
        ...
        |
        <SelectedHosts selectedHosts={selectedHosts} allHosts={allHosts} changeSelectedHosts={selectedHosts => {
