...
const PageTable: React.FC<Props> = ({
  ...
  const getRecordingRules = async () => {
    ...
    const {
      success,
      dat
    } = await getRecordingRuleSubList(curBusiItem.id);
  ...
  const filterData = () => {
    ...
    const res = data.filter(item => {
      ...
      return (item.name.toLowerCase().indexOf(lowerCaseQuery) > -1 || item.append_tags.join(' ').toLowerCase().indexOf(lowerCaseQuery) > -1) && (clusters && pareArray(item.cluster, clusters) || clusters?.length === 0);
    });
  ...
  const handleImportStrategy = async d => {
    const {
      dat
    } = await addOrEditRecordingRule(d, curBusiItem.id, 'Post');
    return dat || {};
  };
  ...
  return <div className='strategy-table-content'>
      ...
      defaultPageSize: 30
    }} loading={loading} dataSource={currentStrategyData} rowSelection={{
      ...
      <ImportAndDownloadModal status={modalType} onClose={() => {
      setModalType(ModalStatus.None);
    }} onSuccess={() => {
      getRecordingRules();
    }} onSubmit={handleImportStrategy} title={t('记录规则')} exportData={exportData} />
