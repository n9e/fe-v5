...
function tagRender(content) {
  ...
  return isCorrectFormat && isLengthAllowed ? <Tag closable={content.closable} onClose={content.onClose} // style={{ marginTop: '2px' }}
  >
      {content.value}
...
const operateForm: React.FC<Props> = ({
  ...
  const addSubmit = () => {
    form.validateFields().then(async values => {
      ...
      if (type === 1) {
        ...
        const {
          dat
        } = await addOrEditRecordingRule(reqBody, curBusiItem.id, method);
        let errorNum = 0;
  ...
  return <div className='operate_con'>
      <Form {...layout} form={form} className='strategy-form' layout={refresh ? 'horizontal' : 'horizontal'} initialValues={{
          ...
          <Card title={t('基本配置')}>
            ...
            message: t('生效集群不能为空')
              ...
              <Select suffixIcon={<CaretDownOutlined />} mode='multiple' onChange={handleClusterChange}>
                ...
                {clusterList?.map(item => <Option value={item} key={item}>
                    {item}
                  </Option>)}
              </Select>
            </Form.Item>
            ...
            <Form.Item noStyle shouldUpdate={(prevValues, curValues) => prevValues.cluster !== curValues.cluster}>
              ...
              return <Form.Item label='PromQL' className={'Promeql-content'} required>
                    <Form.Item name='prom_ql' validateTrigger={['onBlur']} trigger='onChange' rules={[{
                  required: true,
                  message: t('请输入PromQL')
                }]}>
            ...
            <Form.Item required label={t('执行频率')}>
              <Space>
                <Form.Item style={{
                marginBottom: 0
              }} name='prom_eval_interval' initialValue={30} wrapperCol={{
                span: 10
              }} rules={[{
                required: true,
                message: t('执行频率不能为空')
