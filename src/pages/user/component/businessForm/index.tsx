...
const TeamForm = React.forwardRef<ReactNode, TeamProps>((props, ref) => {
  ...
  return !loading ? <Form {...layout} form={form} initialValues={initialValues} preserve={false} layout={refresh ? 'horizontal' : 'horizontal'}>
      {action !== ActionType.AddBusinessMember && <>
          <Form.Item label={t('业务组名称')} name='name' rules={[{
        required: true,
        message: t('业务组名称不能为空！')
      }]}>
            <Input />
          </Form.Item>
          <Form.Item label={t('作为标签使用')} name='label_enable' valuePropName='checked' tooltip={{
        title: t("系统会自动把业务组的英文标识作为标签附到该业务组下辖监控对象的时序数据上"),
        getPopupContainer: () => document.body
          ...
          return getFieldValue('label_enable') && <Form.Item label={t('英文标识')} name='label_value' rules={[{
            required: true
          }]} tooltip={{
            title: <span>
                          {t("尽量用英文，不能与其他业务组标识重复，系统会自动生成")}<Tag color='purple'>busigroup={form.getFieldValue('label_value')}</Tag> {t("的标签")}
                       </span>,
            getPopupContainer: () => document.body
          }}>
                    <Input onChange={val => {
      ...
      {(action === ActionType.CreateBusiness || action === ActionType.AddBusinessMember) && <Form.Item label={t('团队')} required // tooltip={{
    //   title: '默认可读勾选可写',
    // }}
    >
          ...
          remove
            ...
            marginBottom: 8
                    ...
                    <Form.Item style={{
              width: 450
            }} {...restField} name={[name, 'user_group_id']} fieldKey={[fieldKey, 'user_group_id']} rules={[{
              required: true,
              message: t('业务组团队不能为空！')
            }]}>
                      <Select suffixIcon={<CaretDownOutlined />} style={{
                width: '100%'
              }} filterOption={false} onSearch={e => debounceFetcher(e)} showSearch onBlur={() => getList('')}>
                        {userTeam.map(team => <Option key={team.id} value={team.id}>
                            {team.name}
                          </Option>)}
                      </Select>
                    </Form.Item>
                    <Form.Item {...restField} name={[name, 'perm_flag']} fieldKey={[fieldKey, 'perm_flag']} valuePropName='checked'>
                      <Switch checkedChildren={t("读写")} unCheckedChildren={t("只读")} />
                    </Form.Item>
