...
const Resource: React.FC = () => {
  ...
  const teamMemberColumns: ColumnsType<any> = [{
    ...
    title: t('操作'),
    width: '100px',
    render: (text: string, record) => <a style={{
  ...
  return <PageLayout title={t('业务组管理')} icon={<UserOutlined />} hideCluster>
        ...
        height: '100%'
          ...
          <div className='left-tree-area'>
            ...
            margin: '5px 0px 12px'
              ...
              <Input prefix={<SearchOutlined />} placeholder={t('搜索业务组名称')} onPressEnter={e => {
              // @ts-ignore
          ...
          {teamList.length > 0 ? <div className='resource-table-content'>
              ...
              display: 'inline'
                ...
                marginLeft: '8px',
                fontSize: '14px'
              }} onClick={() => handleClick(ActionType.EditBusiness, teamId)}></EditOutlined>
              ...
              <Row justify='space-between' align='middle'>
                <Col span='12'>
                  <Input prefix={<SearchOutlined />} value={searchMemberValue} className={'searchInput'} onChange={e => setSearchMemberValue(e.target.value)} placeholder={t('搜索团队名称')} />
                </Col>
              ...
              <Table rowKey='id' columns={teamMemberColumns} dataSource={memberList && memberList.length > 0 ? memberList.filter(item => item.user_group && item.user_group.name.indexOf(searchMemberValue) !== -1) : []} loading={memberLoading} />
            ...
            fontWeight: 'bold'
              ...
              <p>
                {t("业务组（监控对象、监控大盘、告警规则、自愈脚本都要归属某个业务组）为空，请先")} 
               <a onClick={() => handleClick(ActionType.CreateBusiness)}>{t("创建业务组")}</a>
              </p>
            </div>}
        </div>
      </div>
      <UserInfoModal visible={visible} action={(action as ActionType)} userType={'business'} onClose={handleClose} teamId={teamId} onSearch={val => {
      setTeamId(val);
    }} />
    </PageLayout>;
};
