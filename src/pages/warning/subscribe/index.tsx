...
const Shield: React.FC = () => {
  ...
  const columns: ColumnsType = [{
    ...
    render: (text: string, record: subscribeItem) => {
      return <>
            {record.user_groups?.map(item => <ColorTag text={item.name} key={item.id}></ColorTag>)}
          </>;
    }
  }, {
    title: t('编辑人'),
  ...
  return <PageLayout title={t('订阅规则')} icon={<CopyOutlined />} hideCluster>
      <div className='shield-content'>
        ...
        {curBusiItem?.id ? <div className='shield-index'>
          ...
          </div> : <BlankBusinessPlaceholder text={t("订阅规则")} />}
      </div>
    </PageLayout>;
};
