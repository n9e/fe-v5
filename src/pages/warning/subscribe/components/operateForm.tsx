...
const OperateForm: React.FC<Props> = ({
  ...
  const notifyGroupsOptions = (detail.user_groups ? detail.user_groups.filter(item => !notifyGroups.find(i => item.id === i.id)) : []).concat(notifyGroups).map((ng: any) => <Option value={String(ng.id)} key={ng.id}>
      {ng.name}
    </Option>);
  ...
  const onFinish = values => {
    ...
    if (type === 1) {
      ...
      addSubscribe(params, curBusiItem.id).then(_ => {
  ...
  return <>
      <div className='operate-form-index' id={littleAffect ? 'littleAffect' : ''}>
        <Form form={form} {...layout} layout='vertical' className='operate-form' onFinish={onFinish} onFinishFailed={onFinishFailed} initialValues={{
        ...detail,
        cluster: detail.cluster ? detail.cluster.split(' ') : clusterList || ['Default'],
        // 生效集群
        ...
        new_channels: detail?.new_channels?.split(' ')
          ...
          <Card>
            <Form.Item label={t('生效集群：')} name='cluster' rules={[{
            required: true,
            message: t('生效集群不能为空')
          }]}>
              <Select suffixIcon={<CaretDownOutlined />} mode='multiple' onChange={handleClusterChange}>
                ...
                {clusterList?.map((item, index) => <Option value={item} key={index}>
                    {item}
                  </Option>)}
              </Select>
            </Form.Item>
            <Form.Item label={t('订阅告警规则：')}>
              {!!ruleCur?.id && <Button type='primary' ghost style={{
              marginRight: '8px'
            }} onClick={() => {
              ...
              {!!ruleCur?.id && <DeleteOutlined style={{
              cursor: 'pointer',
              fontSize: '18px',
              marginLeft: 5
            }} onClick={() => subscribeRule({})} />}
            </Form.Item>
            ...
            <Form.List name='tags' initialValue={[{}]}>
              ...
              remove
                  ...
                  {fields.map((field, index) => <TagItem field={field} fields={fields} key={index} remove={remove} add={add} form={form} />)}
                  <Form.Item>
            ...
            <Form.Item label={t('新的告警级别：')} name='new_severity' initialValue={2} style={{
            display: form.getFieldValue('redefine_severity') ? 'block' : 'none'
          }}>
              <Radio.Group>
                <Radio key={1} value={1}>
                  {t('一级报警')}
            ...
            <Form.Item label={t('新的通知媒介：')} name='new_channels' style={{
            display: form.getFieldValue('redefine_channels') ? 'block' : 'none'
          }}>
              <Checkbox.Group>
                {contactList.map((c: {
                key: string;
            ...
            <Form.Item label={t('订阅告警接收组：')} name='user_group_ids' rules={[{
            required: true,
            message: t('告警接收组不能为空')
          }]}>
        ...
        <RuleModal visible={ruleModalShow} ruleModalClose={() => {
        setRuleModalShow(false);
