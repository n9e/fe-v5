...
export default function GraphCpt(props: IProps) {
  ...
  useEffect(() => {
    ...
    if (rid && uuid) {
      getBrainData({
        ...
        const dat = _.map(_.filter(res.data, item => {
  ...
  return <Drawer title={t("曲线详情")} width={800} placement='right' onClose={() => {
    setVisible(false);
  }} visible={visible}>
      <div>
        <div style={{
        ...
        alignItems: 'center'
          ...
          <Space>
            <TimeRangePickerWithRefresh value={range} onChange={setRange} refreshTooltip={`${t("刷新间隔小于 step(")}${getStepByTimeAndStep(range, step)}${t("s) 将不会更新数据")}`} />
