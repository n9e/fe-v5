...
const operateForm: React.FC<Props> = ({
  ...
  const contactListCheckboxes = contactList.map((c: {
    key: string;
    label: string;
  }) => <Checkbox value={c.key} key={c.label}>
      {c.label}
    </Checkbox>);
  const notifyGroupsOptions = notifyGroups.map((ng: any) => <Option value={String(ng.id)} key={ng.id}>
      {ng.name}
    </Option>);
  ...
  const addSubmit = () => {
    form.validateFields().then(async values => {
      ...
      if (type === 1) {
        ...
        const {
          dat
        } = await addOrEditStrategy(reqBody, curBusiItem.id, method);
        let errorNum = 0;
  ...
  return <div className='operate_con'>
      <Form form={form} className='strategy-form' layout='vertical' initialValues={{
      severity: 2,
      disabled: 0,
      // 0:立即启用 1:禁用  待修改
      ...parseValues(detail),
      ...
      effective_time: detail?.enable_etimes ? detail?.enable_etimes.map((item, index) => ({
      ...
      // 1:启用 0:禁用
      callbacks: !_.isEmpty(detail?.callbacks) ? detail.callbacks.map(item => ({
        url: item
      })) : [{}]
    }}>
        ...
        width: '100%'
          ...
          <Card title={t('基本配置')}>
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item label={t('规则标题：')} name='name' rules={[{
                required: true,
                message: t('规则标题不能为空')
              }]}>
            ...
            <Form.Item label={t('告警级别')} name='severity' rules={[{
            required: true,
            message: t('告警级别不能为空')
          }]}>
              <Radio.Group>
            ...
            <Form.Item shouldUpdate={(prevValues, curValues) => prevValues.cate !== curValues.cate} noStyle>
              ...
              if (cate === 'prometheus') {
                return <>
                      ...
                      return <Form.Item label='PromQL' className={'Promeql-content'} required style={{
                        marginBottom: 0
                      }}>
                              <AdvancedWrap var='VITE_IS_ALERT_AI'>
                                {visible => {
                            ...
                            return <Input.Group compact>
                              ...
                              }} name='prom_ql' validateTrigger={['onBlur']} trigger='onChange' rules={[{
                                required: true,
                                message: t('请输入PromQL')
                              }]}>
                                        <PromQLInput url='/api/n9e/prometheus' headers={{
                                  'X-Cluster': cluster,
                                  ...
                                  setIsChecked(false);
                                      ...
                                      {visible && getFieldValue('algorithm') === 'holtwinters' && <Button onClick={() => {
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item shouldUpdate={(prevValues, curValues) => prevValues.cate !== curValues.cate} noStyle>
                  ...
                  const cate = getFieldValue('cate');
                  return <Form.Item name='prom_eval_interval' label={t('执行频率（秒）')} tooltip={cate === 'prometheus' ? t(`${t("每隔")}${form.getFieldValue('prom_eval_interval')}${t("秒，把PromQL作为查询条件，去查询后端存储，如果查到了数据就表示当次有监控数据触发了规则")}`) : t("每隔15秒，去查询后端存储")} initialValue={60} rules={[{
                    required: true,
                    message: t('执行频率不能为空')
                  }]}>
              ...
              <Col span={12}>
                <Form.Item shouldUpdate={(prevValues, curValues) => prevValues.cate !== curValues.cate} noStyle>
                  ...
                  const cate = getFieldValue('cate');
                  return <Form.Item name='prom_for_duration' label={t('持续时长（秒）')} tooltip={cate === 'prometheus' ? t(`${t("通常持续时长大于执行频率，在持续时长内按照执行频率多次执行PromQL查询，每次都触发才生成告警；如果持续时长置为0，表示只要有一次PromQL查询触发阈值，就生成告警")}`) : t("通常持续时长大于执行频率，在持续时长内按照执行频率多次执行查询，每次都触发才生成告警；如果持续时长置为0，表示只要有一次查询的数据满足告警条件，就生成告警")} initialValue={60} rules={[{
                    required: true,
                    message: t('持续时长不能为空')
                  }]}>
            ...
            <Form.Item label={t("附加标签")} name='append_tags' rules={[{
            required: false,
            message: t("请填写至少一项标签！")
          }, isValidFormat]}>
          ...
          <Card title={t('生效配置')}>
            <Form.Item label={t('立即启用')} name='enable_status' rules={[{
            required: true,
            message: t('立即启用不能为空')
            ...
            <Form.List name='effective_time'>
              ...
              remove
                  ...
                  width: 450
                      ...
                      {t("生效时间")}<PlusCircleOutlined className='control-icon-normal' onClick={() => add()} />
                    </div>
                    <div style={{
                  width: 110
                }}>{t("开始时间")}</div>
                ...
                marginBottom: 8
                      ...
                      <Form.Item {...restField} name={[name, 'enable_days_of_week']} style={{
                  width: 450
                }} rules={[{
                  required: true,
                  message: t('请选择生效周期')
                      ...
                      <Form.Item {...restField} name={[name, 'enable_stime']} style={{
                  width: 110
                }} rules={[{
                  required: true,
                  message: t('开始时间不能为空')
                      ...
                      <Form.Item {...restField} name={[name, 'enable_etime']} style={{
                  width: 110
                }} rules={[{
                  required: true,
                  message: t('结束时间不能为空')
            ...
            <Form.Item shouldUpdate={(prevValues, curValues) => prevValues.cate !== curValues.cate} noStyle>
              ...
              if (getFieldValue('cate') === 'prometheus') {
                return <Form.Item label={t('仅在本业务组生效')} name='enable_in_bg' valuePropName='checked'>
          ...
          <Card title={t('通知配置')}>
            ...
            <Form.Item label={t('启用恢复通知')}>
              <Space>
                <Form.Item name='notify_recovered' valuePropName='checked' style={{
                marginBottom: 0
              }}>
                  <Switch />
                </Form.Item>
            ...
            <Row gutter={16}>
              <Col span={8}>
                <Form.Item label={t('留观时长（秒）')} name='recover_duration' initialValue={0} tooltip={t(`${t("持续")}${form.getFieldValue('recover_duration')}${t("秒没有再次触发阈值才发送恢复通知")}`)}>
                  <InputNumber min={0} style={{
                  width: '100%'
                }} />
                </Form.Item>
              ...
              <Col span={8}>
                <Form.Item label={t('重复发送频率（分钟）')} name='notify_repeat_step' initialValue={60} rules={[{
                required: true,
              ...
              <Col span={8}>
                <Form.Item label={t('最大发送次数')} name='notify_max_number' initialValue={0} rules={[{
                required: true,
                message: t('最大发送次数不能为空')
              }]} tooltip={t(`${t("如果值为0，则不做最大发送次数的限制")}`)}>
                  <InputNumber min={0} precision={0} style={{
                  width: '100%'
                }} />
                </Form.Item>
              </Col>
            ...
            <Form.List name='callbacks'>
              ...
              remove
                  ...
                  <div>
                    {t("回调地址")}<PlusCircleOutlined className='control-icon-normal' onClick={() => add()} />
                  </div>
                  {fields.map(field => <Row gutter={16} key={field.key}>
                      <Col flex='auto'>
          ...
          marginTop: 20
            ...
            <Button type='primary' onClick={addSubmit} style={{
            marginRight: '8px'
          }}>
              {type === 1 ? t('编辑') : type === 2 ? t('克隆') : t('创建')}
            </Button>
            {type === 1 && <Button danger style={{
            marginRight: '8px'
          }} onClick={() => {
            Modal.confirm({
              title: t('是否删除该告警规则?'),
