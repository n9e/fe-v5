...
const TagItem: React.FC<Itag> = ({
  ...
  const funcChange = val => {
    let text = '';
    if (val === 'in') {
      text = t("可以输入多个值，用回车分割");
    } else if (val === '=~') {
      text = t("请输入正则表达式匹配标签value");
    }
    setfuncCur(val);
  ...
  return <>
      ...
      marginBottom: '10px'
        ...
        <Col span={5}>
          <Form.Item style={{
          marginBottom: 0
        }} name={[field.name, 'key']} fieldKey={[field.name, 'key']} rules={[{
          required: true,
          message: t('key不能为空')
        }]}>
        ...
        <Col span={3}>
          <Form.Item style={{
          marginBottom: 0
        }} name={[field.name, 'func']} fieldKey={[field.name, 'func']} initialValue='=='>
        ...
        <Col span={15}>
          <Form.Item style={{
          marginBottom: 0
        }} name={[field.name, 'value']} fieldKey={[field.name, 'value']} rules={[{
          required: true,
          message: t('value不能为空')
        }]}>
            {['not in', 'in'].includes(funcCur) ? <Select mode='tags' open={false} style={{
            width: '100%'
          }} placeholder={t(valuePlaceholder)} tokenSeparators={[' ']}></Select> : <Input className='ant-input' placeholder={t(valuePlaceholder)} />}
          </Form.Item>
        </Col>
        <Col>
