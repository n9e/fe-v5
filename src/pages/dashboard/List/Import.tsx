...
function Import(props: IProps & ModalWrapProps) {
  ...
  return <Modal className='dashboard-import-modal' title={<Tabs activeKey={modalType} onChange={(e: ModalType) => setModalType(e)} className='custom-import-alert-title'>
      ...
      position: 'relative'
        ...
        color: '#1890ff'
              ...
              </div>} key='ImportGrafana'></TabPane>
        </Tabs>} visible={visible} onCancel={() => {
    refreshList();
    destroy();
  }} footer={null}>
      {modalType === 'BuiltIn' ? <>
          <Input prefix={<SearchOutlined />} placeholder={t("请输入要查询的大盘名称")} value={searchVal} onChange={e => {
        setSearchVal(e.target.value);
      }} />
          <Table style={{
        marginTop: 10
      }} rowKey='name' size='small' dataSource={filteredBuildinList} columns={[{
        title: t("大盘名称"),
        dataIndex: 'name'
      }, {
        title: t("操作"),
      ...
      }]} pagination={filteredBuildinList.length < 5 ? false : {
        pageSize: 5
      }} />
        </> : null}
      {modalType === 'Import' ? <Form layout='vertical' onFinish={vals => {
      const data = getValidImportData(vals.import);
      createDashboard(busiId, { ...data,
        ...
        destroy();
          ...
          <Form.Item label={t("大盘JSON")} name='import' rules={[{
        required: true,
        message: t("请输入大盘JSON")
      }]}>
      ...
      {modalType === 'ImportGrafana' ? <Form layout='vertical' onFinish={vals => {
