...
export default function DetailV2({
  ...
  const {
    clusters
  } = useSelector<CommonRootState, CommonStoreState>(state => state.common);
  ...
  const [range, setRange] = useState<IRawTimeRange>(getDefaultValue(dashboardTimeCacheKey, {
    start: 'now-1h',
    end: 'now'
  }));
  ...
  const refresh = (cbk?: () => void) => {
    getDashboard(id).then(res => {
      ...
      if (res.configs) {
        const configs = JSONParse(res.configs); // TODO: configs 中可能没有 var 属性会导致 VariableConfig 报错

        const variableConfig = configs.var ? configs : { ...configs,
          var: []
        };
        setVariableConfig(_.map(variableConfig.var, item => {
  ...
  const handleVariableChange = (value, b, valueWithOptions) => {
    ...
    dashboardConfigs.var = value; // 更新变量配置

    b && handleUpdateDashboardConfigs(dashboard.id, {
  ...
  return <PageLayout customArea={<Title isPreview={isPreview} curCluster={curCluster} clusters={clusters} setCurCluster={setCurCluster} dashboard={dashboard} setDashboard={setDashboard} refresh={() => {
    // 集群修改需要刷新数据
    refresh(() => {
  ...
  }} range={range} setRange={v => {
    setRange(v);
  }} step={step} setStep={setStep} refreshRef={refreshRef} onAddPanel={type => {
    if (type === 'row') {
      const newPanels = updatePanelsInsertNewPanelToGlobal(panels, {
        type: 'row',
        id: uuidv4(),
        name: t("分组"),
        collapsed: true
        ...
        <div className='dashboard-detail-content'>
          ...
          <div className='dashboard-detail-content-header'>
            <div className='variable-area'>
              {variableConfig && <VariableConfig isPreview={isPreview} onChange={handleVariableChange} value={variableConfig} cluster={curCluster} range={range} id={id} onOpenFire={stopAutoRefresh} />}
            </div>
            {!isPreview && <DashboardLinks value={dashboardLinks} onChange={v => {
            const dashboardConfigs: any = JSONParse(dashboard.configs);
          ...
          {variableConfigWithOptions && <Panels isPreview={isPreview} key={forceRenderKey} editable={editable} panels={panels} setPanels={setPanels} curCluster={curCluster} dashboard={dashboard} range={range} step={step} variableConfig={variableConfigWithOptions} onShareClick={panel => {
          const serielData = {
            dataProps: { ...panel,
      ...
      <Editor mode='add' visible={editorData.visible} setVisible={visible => {
      setEditorData({ ...editorData,
        visible
      });
    }} variableConfigWithOptions={variableConfigWithOptions} cluster={curCluster} id={editorData.id} time={range} initialValues={editorData.initialValues} onOK={values => {
