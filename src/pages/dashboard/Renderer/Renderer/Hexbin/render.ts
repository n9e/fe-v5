...
function renderHoneyComb(
  svgGroup,
  data,
  { width, height, fontAutoScale = true, fontSize = 12 }
) {
  ...
  const offSetX = hexRadius * Math.cos((30 * Math.PI) / 180);
  const adjustedOffSetX = offSetX + (width - offSetX * 2 * mapColumns - offSetX) / 2;
  ...
  hexagons
    ...
    if (activeLabelFontSize) {
      hexagons.enter().insert("rect", "text")
