...
function Item(props) {
  ...
  return <div className='renderer-bar-gauge-item' key={item.name}>
      <Tooltip title={item.name}>
        <div className='renderer-bar-gauge-item-name' style={{
        width: `${serieWidth}%`
      }}>
          {item.name}
        </div>
      </Tooltip>
      <div className='renderer-bar-gauge-item-value' style={{
      width: `${100 - serieWidth}%`
    }}>
        <div className='renderer-bar-gauge-item-value-bg' style={{
        backgroundColor: themeMode === 'dark' ? '#20222E' : '#F6F6F6'
      }} />
        <div ref={bgRef} className='renderer-bar-gauge-item-value-color-bg' style={{
        color: themeMode === 'dark' ? '#fff' : '#20222E',
        borderRight: `2px solid ${color}`,
        backgroundColor: Color(color).alpha(displayMode === 'basic' ? 0.2 : 1).rgb().string(),
        width: `${item.stat / maxValue * 100}%`
      }}>
          {displayMode === 'basic' && <div ref={textRef} className='renderer-bar-gauge-item-value-text' style={{
          color: color,
          right: getTextRight()
        }}>
              {item.text}
...
export default function BarGauge(props: IProps) {
  ...
  let calculatedValues = getCalculatedValuesBySeries(series, calc, {
  ...
  return <div className='renderer-bar-gauge-container'>
