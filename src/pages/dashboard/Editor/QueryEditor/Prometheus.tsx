      ...
      return <>
            ...
            return <Panel header={<Form.Item noStyle shouldUpdate>
                ...
                alignItems: 'center'
                      ...
                      <Form.Item label='PromQL' {...field} name={[field.name, 'expr']} validateTrigger={['onBlur']} rules={[{
                  required: true,
                  message: t("请输入PromQL")
                }]} style={{
                  flex: 1
                }}>
                        <PromQLInput validateTrigger={['onBlur']} url='/api/n9e/prometheus' headers={{
                    ...
                    <Row gutter={10}>
                      <Col flex='auto'>
                        <Form.Item label='Legend' {...field} name={[field.name, 'legend']} tooltip={{
                    ...
                    title: 'Controls the name of the time series, using name or pattern. For example {{hostname}} will be replaced with label value for the label hostname.'
                      ...
                      <Col flex='116px'>
                        <Form.Item label={t("时间选择")} {...field} name={[field.name, 'time']} tooltip={{
                    getPopupContainer: () => document.body,
                    ...
                    return {
                      start: isMathString(val.start) ? val.start : moment(val.start).format('YYYY-MM-DD HH:mm:ss'),
                          ...
                          <TimeRangePicker dateFormat='YYYY-MM-DD HH:mm:ss' allowClear onClear={() => {
                      ...
                      <Col flex='72px'>
                        <Form.Item label='Step' {...field} name={[field.name, 'step']} tooltip={{
                    getPopupContainer: () => document.body,
