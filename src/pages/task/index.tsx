...
const index = (_props: any) => {
  ...
  const {
    tableProps
  } = useAntdTable(options => getTableData(options, curBusiItem.id, query, mine, days), {
    refreshDeps: [curBusiItem.id, query, mine, days]
  });
  const columns: ColumnProps<DataItem>[] = [{
    ...
    render: (text, record) => {
      return <Link to={{
        pathname: `/job-tasks/${record.id}/result`
      }}>{text}</Link>;
    }
  }, {
    ...
    render: (_text, record) => {
      return <span>
          ...
          search: `task=${record.id}`
        }}>{t('task.clone')}</Link>
            <Divider type='vertical' />
            <Link to={{
          pathname: `/job-tasks/${record.id}/detail`
        }}>{t('task.meta')}</Link>
          </span>;
    }
  }, {
      ...
      display: 'flex'
        ...
        padding: 10
            ...
            <Row>
              ...
              width: 200,
              marginRight: 10
            }} prefix={<SearchOutlined />} defaultValue={query} onPressEnter={e => {
              setQuery(e.currentTarget.value);
            }} placeholder={t("搜索标题")} />
                <Select style={{
              marginRight: 10
            }} value={days} onChange={(val: number) => {
              setDays(val);
            }}>
            ...
            <Table rowKey='id' columns={(columns as any)} {...(tableProps as any)} pagination={({ ...tableProps.pagination,
          ...
          </div> : <BlankBusinessPlaceholder text={t('执行历史')}></BlankBusinessPlaceholder>}
      </div>
    </PageLayout>;
};
