...
export default function Chart() {
  ...
  const initChart = () => {
    GetTmpChartData(ids).then(res => {
      let data = res.dat.filter(item => !!item).map(item => {
      ...
      setRange(data[0].dataProps.range); // TODO: 处理当前选中集群不在集群列表的情况
  ...
  const clusterMenu = <Menu selectedKeys={[curCluster]}>
  ...
  return <div className='chart-container'>
      {chartData && chartData.length > 0 && curCluster ? <>
          <div className='chart-container-header'>
            ...
            <div className='right'>
              <Space>
                ...
                <TimeRangePickerWithRefresh refreshTooltip={`${t("刷新间隔小于 step(")}${getStepByTimeAndStep(range, step)}${t("s) 将不会更新数据")}`} onChange={setRange} value={range} />
                <Resolution onChange={v => setStep(v)} initialValue={step} />
                {!semver.valid(chartData[0].dataProps?.version) && <Radio.Group options={[{
              label: <LineChartOutlined />,
              value: ChartType.Line
            }, {
              label: <AreaChartOutlined />,
              value: ChartType.StackArea
            }]} onChange={e => {
              e.preventDefault();
              setChartType(e.target.value);
            }} value={chartType} optionType='button' buttonStyle='solid' />}
              </Space>
            </div>
          </div>
        ...
        if (semver.valid(item.dataProps?.version)) {
          return <div style={{
            ...
            border: '1px solid #efefef'
                  ...
                  <Renderer dashboardId={item.id} key={index} time={range} step={step} values={_.merge({}, item.dataProps, {
              options: {
                legend: {
                  displayMode: 'table'
                }
        ...
        const newItem = { ...item.dataProps,
          range,
          step,
          chartType,
          title: <Tooltip placement='bottomLeft' title={() => <div>
        ...
        return <Graph ref={item.ref} key={index} data={{ ...newItem
        }} graphConfigInnerVisible={false} isShowShare={false} highLevelConfig={item.highLevelConfig || {}} />;
      })}
        </> : <h2 className='holder'>
          <FieldNumberOutlined style={{
        fontSize: '30px'
      }} />
