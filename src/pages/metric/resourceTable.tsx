...
import type { resourceStoreState, resourceItem } from '@/store/businessInterface';
...
function ResourceTable({
  ...
  return <div>
      <div className='table-search-area'>
        <SearchInput placeholder={t('资源信息或标签')} onSearch={handleSearchResource}></SearchInput>
      </div>
      <BaseTable columns={columns} rowKey={'ident'} fetchParams={{
      id: currentGroup?.id || '',
      query
    }} fetchHandle={getResourceList} rowSelection={{
      ...
      simple: true
    }} onFetchList={handleFetchList}></BaseTable>
    </div>;
}
