...
export default function LogstoreSelect(props: IProps) {
  ...
  const label = <span>
      ...
      <Tooltip title={<div>
           ...
           <a href='https://help.aliyun.com/document_detail/48874.htm' target='_blank' style={{
        color: '#c6b2fd'
      }}>
              {t("日志库")}
           </a>
  ...
  if (layout === 'vertical') {
    return <Form.Item {...prefixField} label={label} name={[...prefixName, 'logstore']} rules={[{
      required: true,
      message: t("请输入日志库")
    }]} style={{
      width
  ...
  return <InputGroupWithFormItem label={label} labelWidth={84}>
      <Form.Item {...prefixField} name={[...prefixName, 'logstore']} rules={[{
      required: true,
      message: t("请输入日志库")
    }]} style={{
      width
