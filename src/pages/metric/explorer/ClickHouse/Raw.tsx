...
function Raw(props: IProps, ref) {
  ...
  useImperativeHandle(ref, () => ({
    fetchData: (datasourceCate, datasourceName, values) => {
      ...
      setLoading(true);
      logQuery(requestParams).then(res => {
        setLogs(res.list);
      }).finally(() => {
        setLoading(false);
      });
      getHistogram(requestParams).then(res => {
        setHistogram(_.map(res, item => {
          return {
            metric: item.metric,
            data: item.values
          };
  ...
  return <Spin spinning={loading}>
      {!_.isEmpty(logs) ? <div className='sls-discover-content'>
          ...
          <div className='sls-discover-main'>
            <div className='sls-discover-chart'>
              <div className='sls-discover-chart-content'>
                <Timeseries series={histogram} values={({
              custom: {
                drawStyle: 'bar',
            ...
            } as any)} onClick={(event, datetime, value, points) => {
            ...
            <RawTable data={logs} timeField={timeField} selectedFields={selectedFields} scroll={{
          x: 'max-content',
          y: !isMore ? 312 - 35 : 312
        }} />
          </div>
