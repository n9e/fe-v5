...
const Panel = ({
  ...
  const [datasourceCate, setDatasourceCate] = useState(params.get('data_source_name') ? params.get('data_source_name') : params.get('data_source_id') ? DatasourceCateEnum.elasticsearch : localStorage.getItem('datasource_cate') || DatasourceCateEnum.prometheus);
  useEffect(() => {
  ...
  return <Card bodyStyle={{
    padding: 16
      ...
      datasourceName: getDefaultDatasourceName(datasourceCate, datasourceList)
        ...
        <Space align='start'>
          <InputGroupWithFormItem label={t("数据源类型")} labelWidth={84}>
            ...
            return <Form.Item name='datasourceCate' noStyle>
                ...
                form.setFieldsValue({
                  datasourceName: getDefaultDatasourceName(val, datasourceList)
                      ...
                      {_.map(isES ? datasourceCatesMap.all : datasourceCatesMap.normal, item => <Select.Option key={item.value} value={item.value}>
                          {item.label}
                        </Select.Option>)}
                    </Select>
                  </Form.Item>;
          ...
          <Form.Item shouldUpdate={(prev, curr) => prev.datasourceCate !== curr.datasourceCate} noStyle>
            ...
            return <Input.Group compact>
                ...
                lineHeight: '32px'
                  ...
                  <Form.Item name='datasourceName' rules={[{
                required: cate !== 'prometheus',
                message: t("请选择数据源")
              }]}>
                    <Select placeholder={t("选择数据源")} style={{
                  minWidth: 70
                }} dropdownMatchSelectWidth={false} onChange={(val: string) => {
                  setDefaultDatasourceName(cate, val);
                }}>
                      {_.map(datasourceList[cate], item => <Select.Option value={item} key={item}>
                          {item}
                        </Select.Option>)}
                    </Select>
                  </Form.Item>
      ...
      <span className='remove-panel-btn' onClick={() => {
      removePanel(id);
    }}>
        <CloseCircleOutlined />
      </span>
...
const MetricExplorerPage = () => {
  ...
  return <PageLayout title={t("即时查询")} icon={<LineChartOutlined />} hideCluster>
