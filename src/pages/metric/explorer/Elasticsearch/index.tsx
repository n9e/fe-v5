...
export default function index(props: IProps) {
  ...
  const fetchSeries = (values) => {
    if (timesRef.current) {
      ...
      metricQuery({ ...timesRef.current, datasourceCate: 'elasticsearch', datasourceName: values.datasourceName, query: values.query, interval, intervalUnit }).then((res) => {
  ...
  const fetchData = (page) => {
    form.validateFields().then((values) => {
      ...
      getLogsQuery({
        ...
        query: [{ ...timesRef.current, index: values.query.index, filter: values.query.filter, date_field: values.query.date_field, limit: LOGS_LIMIT, page }],
      })
        .then((res) => {
