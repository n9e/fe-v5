...
const MetricTable = ({
    ...
    GetMetrics(queryData).then(res => {
      if (res.dat.metrics) {
        ...
        if (initVal && initVal['name']) {
          ...
          let isMetricsHave = res.dat.metrics.find(e => e.name === initVal.name);
  ...
  return <>
      <Select mode={multiple ? 'multiple' : undefined} style={{
      width: '100%'
    }} placeholder={t('请输入监控指标(输入4个以上字符发起检索)')} value={selMetrics} onChange={handleSelectChange} onSearch={handleSearch} onDeselect={() => {
      // hack logic, 当选中项被全部删除后需要重置options，但是此时读到的selMetrics仍有1项
      if (selMetrics.length <= 1) {
        getMetrics();
      }
    }} // filterOption={false}
    showSearch dropdownClassName='overflow-586'>
        {metrics.map((item, i) => {
