...
const index = (_props: any) => {
  ...
  const {
    tableProps,
    refresh
  } = useAntdTable<any, any>(options => getTableData(options, busiId, query), {
  ...
  const columns: ColumnProps<Tpl>[] = [{
    ...
    dataIndex: 'tags',
    render: text => {
      return _.map(text, item => <Tag color='purple' key={item} onClick={() => handleTagClick(item)}>
            {item}
          </Tag>);
    }
  }, {
    ...
    render: (_text, record) => {
      return <span>
          ...
          width: 100
        }}>{t('table.delete.sure')}</div>} onConfirm={() => {
        ...
        padding: 20
            ...
            <Row>
              <Col span={14} className='mb10'>
                <Input style={{
              width: 200
            }} ref={searchRef} prefix={<SearchOutlined />} defaultValue={query} onPressEnter={e => {
              ...
              pathname: `/job-tpls/add`
                ...
                <Dropdown overlay={<Menu>
                      <Menu.Item>
                        <Button type='link' disabled={selectedIds.length === 0} onClick={() => {
                  handleBatchBindTags();
                }}>
                          {t('tpl.tag.bind')}
                        </Button>
            ...
            <Table rowKey='id' columns={columns} {...(tableProps as any)} rowSelection={{
          ...
          </div> : <BlankBusinessPlaceholder text={t("自愈脚本")} />}
      </div>
    </PageLayout>;
};
